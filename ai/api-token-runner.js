/**
 * ai/api-token-runner.js
 *
 * Self-healing runner for tests/api-token.spec.ts
 *
 * Flow:
...
const TEST_FILE   = path.join(__dirname, '../tests/api-token.spec.ts');
const REPORT_FILE = path.join(__dirname, '../test-results/api-token-report.json');
      ...
      if (suite.specs) {
        for (const spec of suite.specs) {
          for (const test of (spec.tests || [])) {
            ...
            if (failed) {
              const result =
                test.results.find(r => r.status === 'failed' || r.status === 'timedOut') || {};
              failures.push({
                title: spec.title,
...
- API Token page:   https://aiv.test.oneaiv.com:8086/aiv/ApiTokens
- Token name input: #Name1
- Revoke button:    getByRole('button', { name: 'Revoke' })  — PrimeNG button with p-button-label span`;
...
async function main() {
  console.log('═══════════════════════════════════════════════════════');
  console.log(' Self-Healing Runner — tests/api-token.spec.ts');
  console.log('═══════════════════════════════════════════════════════');
  ...
  while (attempt <= MAX_RETRIES) {
    ...
    if (anyFixed) {
      if (attempt === 1) {
        ...
        console.log(`\n💾 Original backed up to tests/api-token.spec.ts.bak`);
      }
