...
test.describe.serial('AIV Reports - Format PPTX Workflow', () => {

  test.beforeEach(async ({ page }) => {
    await ensureLoggedIn(page);
  });

  test('reports_format_pptx', async ({ page }) => {
    ...
    // Step 4: Change name to Order Details pptx
    console.log('👉 Step 4: Setting Output Name to Order Details pptx...');
    const outputNameInput = schedulerDialog.locator('input[name="soutputname"]').first();
    await outputNameInput.clear();
    await outputNameInput.fill('Order Details pptx');
    await page.waitForTimeout(1000);
    await shot(page, '03-output-name-pptx.png');

    // Step 5: Select ppt/pptx in Format dropdown
    console.log('👉 Step 5: Selecting ppt/pptx in Format dropdown...');
    const formatDropdown = schedulerDialog.locator('p-dropdown[name="outputformat"]').first();
    await formatDropdown.click();
    await page.waitForTimeout(1000);

    const pptOption = page.locator('.p-dropdown-item, [role="option"], li').filter({ hasText: /^(ppt|pptx)$/i }).first();
    await expect(pptOption).toBeVisible({ timeout: 5000 });
    await pptOption.click();
    await page.waitForTimeout(3000);
    await shot(page, '04-format-selected-pptx.png');
    ...
    // Step 19: Verify pptx/ppt file is present (with polling refresh loop)
    console.log('👉 Step 19: Verifying pptx/ppt file is present...');
    const pptFile = page.locator('[role="gridcell"], td, .e-rowcell').filter({ hasText: 'Order Details ppt' }).first();
    
    let fileFound = false;
    const refreshBtn = page.locator('button:has(.pi-refresh), button .pi-refresh, .pi-refresh, [class*="refresh"]').first();
    for (let i = 0; i < 6; i++) {
      if (await pptFile.isVisible().catch(() => false)) {
        fileFound = true;
        break;
      }
      console.log(`👉 ppt/pptx file not visible yet. Refreshing grid (attempt ${i + 1}/6)...`);
    ...
    await expect(pptFile).toBeVisible({ timeout: 15000 });
    await pptFile.click();
    await page.waitForTimeout(1000);
    await shot(page, '12-pptx-file-verified.png');

    console.log('✅ reports_format_pptx completed successfully');
  });
});
