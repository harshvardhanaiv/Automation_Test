...
test.describe.serial('AIV Reports - Shared Suffix Schedule Workflow', () => {
  ...
  test('reports_suffix_yyyy-dd-MM_HH-mm-ss-S', async ({ page }) => {
    ...
    // Step 7: Select yyyy-dd-MM HH-mm-ss-S in Suffix dropdown
    console.log('👉 Step 7: Selecting yyyy-dd-MM HH-mm-ss-S in Suffix dropdown...');
    const suffixDropdown = schedulerDialog.locator('p-dropdown[name="outputsuffix"]').first();
    await suffixDropdown.click();
    await page.waitForTimeout(1000);

    const suffixOption = page.locator('.p-dropdown-item, [role="option"], li').filter({ hasText: /^\s*yyyy-dd-MM HH-mm-ss-S\s*$/ }).first();
    ...
    await expect(suffixFile).toBeVisible({ timeout: 40000 }); // allow extra time for file generation
