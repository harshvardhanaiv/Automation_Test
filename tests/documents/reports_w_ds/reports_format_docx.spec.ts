...
test.describe.serial('AIV Reports - Format DOCX Workflow', () => {

  test.beforeEach(async ({ page }) => {
    await ensureLoggedIn(page);
  });

  test('reports_format_docx', async ({ page }) => {
    ...
    // Step 4: Change name to Order Details docx
    console.log('👉 Step 4: Setting Output Name to Order Details docx...');
    const outputNameInput = schedulerDialog.locator('input[name="soutputname"]').first();
    await outputNameInput.clear();
    await outputNameInput.fill('Order Details docx');
    await page.waitForTimeout(1000);
    await shot(page, '03-output-name-docx.png');

    // Step 5: Select docx in Format dropdown
    console.log('👉 Step 5: Selecting docx in Format dropdown...');
    const formatDropdown = schedulerDialog.locator('p-dropdown[name="outputformat"]').first();
    await formatDropdown.click();
    await page.waitForTimeout(1000);

    const docxOption = page.locator('.p-dropdown-item, [role="option"], li').filter({ hasText: /^docx$/ }).first();
    await expect(docxOption).toBeVisible({ timeout: 5000 });
    await docxOption.click();
    await page.waitForTimeout(3000);
    await shot(page, '04-format-selected-docx.png');
    ...
    const [newTabPage] = await Promise.all([
      page.context().waitForEvent('page', { timeout: 15000 }).catch(() => page),
      runBtn.click()
    ]);
    ...
    // Step 13: Wait 10 seconds for document generation/download
    console.log('👉 Step 13: Waiting 10 seconds for document generation/download...');
    ...
    // Step 19: Verify docx file is present (with polling refresh loop)
    console.log('👉 Step 19: Verifying docx file is present...');
    const docxFile = page.locator('[role="gridcell"], td, .e-rowcell').filter({ hasText: 'Order Details docx' }).first();
    
    let fileFound = false;
    const refreshBtn = page.locator('button:has(.pi-refresh), button .pi-refresh, .pi-refresh, [class*="refresh"]').first();
    for (let i = 0; i < 6; i++) {
      if (await docxFile.isVisible().catch(() => false)) {
        fileFound = true;
        break;
      }
      console.log(`👉 docx file not visible yet. Refreshing grid (attempt ${i + 1}/6)...`);
    ...
    await expect(docxFile).toBeVisible({ timeout: 15000 });
    await docxFile.click();
    await page.waitForTimeout(1000);
    await shot(page, '12-docx-file-verified.png');

    console.log('✅ reports_format_docx completed successfully');
  });
});
