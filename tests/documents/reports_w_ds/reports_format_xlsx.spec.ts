...
test.describe.serial('AIV Reports - Format XLSX Workflow', () => {

  test.beforeEach(async ({ page }) => {
    await ensureLoggedIn(page);
  });

  test('reports_format_xlsx', async ({ page }) => {
    ...
    // Step 4: Change name to Order Details xlsx
    console.log('👉 Step 4: Setting Output Name to Order Details xlsx...');
    const outputNameInput = schedulerDialog.locator('input[name="soutputname"]').first();
    await outputNameInput.clear();
    await outputNameInput.fill('Order Details xlsx');
    await page.waitForTimeout(1000);
    await shot(page, '03-output-name-xlsx.png');

    // Step 5: Select xlsx in Format dropdown
    console.log('👉 Step 5: Selecting xlsx/aiv-xlsx in Format dropdown...');
    const formatDropdown = schedulerDialog.locator('p-dropdown[name="outputformat"]').first();
    await formatDropdown.click();
    await page.waitForTimeout(1000);

    const xlsxOption = page.locator('.p-dropdown-item, [role="option"], li').filter({ hasText: /^\s*xlsx\s*$/ }).first();
    await expect(xlsxOption).toBeVisible({ timeout: 5000 });
    await xlsxOption.click();
    await page.waitForTimeout(3000);
    await shot(page, '04-format-selected-xlsx.png');
    ...
    // Step 19: Verify xlsx file is present (with polling refresh loop)
    console.log('👉 Step 19: Verifying xlsx file is present...');
    const xlsxFile = page.locator('[role="gridcell"], td, .e-rowcell').filter({ hasText: 'Order Details xlsx' }).first();
    
    let fileFound = false;
    const refreshBtn = page.locator('button:has(.pi-refresh), button .pi-refresh, .pi-refresh, [class*="refresh"]').first();
    for (let i = 0; i < 6; i++) {
      if (await xlsxFile.isVisible().catch(() => false)) {
        fileFound = true;
        break;
      }
      console.log(`👉 xlsx file not visible yet. Refreshing grid (attempt ${i + 1}/6)...`);
    ...
    await expect(xlsxFile).toBeVisible({ timeout: 15000 });
    await xlsxFile.click();
    await page.waitForTimeout(1000);
    await shot(page, '12-xlsx-file-verified.png');

    console.log('✅ reports_format_xlsx completed successfully');
  });
});
