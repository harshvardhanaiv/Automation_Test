...
test.describe.serial('AIV Reports - Format HTML Workflow', () => {

  test.beforeEach(async ({ page }) => {
    await ensureLoggedIn(page);
  });

  test('reports_format_html', async ({ page }) => {
    ...
    // Step 4: Change name to Order Details html
    console.log('👉 Step 4: Setting Output Name to Order Details html...');
    const outputNameInput = schedulerDialog.locator('input[name="soutputname"]').first();
    await outputNameInput.clear();
    await outputNameInput.fill('Order Details html');
    await page.waitForTimeout(1000);
    await shot(page, '03-output-name-html.png');

    // Step 5: Select html in Format dropdown
    console.log('👉 Step 5: Selecting html in Format dropdown...');
    const formatDropdown = schedulerDialog.locator('p-dropdown[name="outputformat"]').first();
    await formatDropdown.click();
    await page.waitForTimeout(1000);

    const htmlOption = page.locator('.p-dropdown-item, [role="option"], li').filter({ hasText: /^\s*html\s*$/i }).first();
    await expect(htmlOption).toBeVisible({ timeout: 5000 });
    await htmlOption.click();
    await page.waitForTimeout(3000);
    await shot(page, '04-format-selected-html.png');
    ...
    if (newTabPage !== page) {
      console.log('👉 New tab opened. Waiting for 15 seconds for report execution to finish...');
      await newTabPage.waitForLoadState().catch(() => {});
      await page.waitForTimeout(15000);
      
      if (!newTabPage.isClosed()) {
        await shot(newTabPage, '08-html-report-executed.png').catch(() => {});
        console.log('👉 Closing execution tab to return focus to main page...');
        await newTabPage.close().catch(() => {});
      } else {
        await shot(page, '08-html-report-executed.png');
      }
    } else {
      await shot(page, '08-html-report-executed.png');
    }
    ...
    // Step 16: Verify html file is present (with polling refresh loop)
    console.log('👉 Step 16: Verifying html file is present...');
    const htmlFile = page.locator('[role="gridcell"], td, .e-rowcell').filter({ hasText: 'Order Details html' }).first();
    
    let fileFound = false;
    const refreshBtn = page.locator('button:has(.pi-refresh), button .pi-refresh, .pi-refresh, [class*="refresh"]').first();
    for (let i = 0; i < 6; i++) {
      if (await htmlFile.isVisible().catch(() => false)) {
        fileFound = true;
        break;
      }
      console.log(`👉 html file not visible yet. Refreshing grid (attempt ${i + 1}/6)...`);
    ...
    await expect(htmlFile).toBeVisible({ timeout: 15000 });
    await htmlFile.click();
    await page.waitForTimeout(1000);
    await shot(page, '12-html-file-verified.png');

    console.log('✅ reports_format_html completed successfully');
  });
});
