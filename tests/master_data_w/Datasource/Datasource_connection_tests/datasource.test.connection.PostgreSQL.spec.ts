/**
 * datasource.test.connection.PostgreSQL.spec.ts
 *
 * Datasource creation test — Master Data section
 * Creates a PostgreSQL datasource with connection testing
 */
...
test.describe('Datasource Creation Flow', () => {
  ...
  test('should create a new datasource via Master Data > Datasources', async ({ page }) => {
    ...
    // Step 6: Search for PostgreSQL in the top-right search bar
    await test.step('Search for PostgreSQL datasource', async () => {
      const searchBar = page.getByPlaceholder('Search').filter({ visible: true }).first();
      await expect(searchBar).toBeVisible({ timeout: 10000 });
      await searchBar.fill('PostgreSQL');
      await page.waitForTimeout(1000);

      const postgresCard = page.getByRole('link', { name: /PostgreSQL/i }).first();
      await expect(postgresCard).toBeVisible({ timeout: 10000 });
      await postgresCard.click();
      await waitAndScreenshot(page, '6-postgresql-selected');
    });
    ...
    await test.step('Fill datasource credentials', async () => {
      ...
      // 1. Display Name
      await fillField('Display Name', 'Parth_Playwright_test');
      await waitAndScreenshot(page, '7a-display-name-filled');

      // 2. Class
      await fillField('Class', 'org.postgresql.Driver');
      await waitAndScreenshot(page, '7b-class-filled');

      // 3. Connection URL
      await fillField('Connection URL', 'jdbc:postgresql://178.105.6.171:8100/mydb');
      await waitAndScreenshot(page, '7c-url-filled');

      // 4. Username
      await fillField('Username', 'postgres');
      await waitAndScreenshot(page, '7d-username-filled');

      // 5. Password
      await fillField('Password', 'root');
    ...
    await test.step('Search for datasource by name', async () => {
      const searchBar = page.getByPlaceholder('Search').filter({ visible: true }).first();
      await expect(searchBar).toBeVisible({ timeout: 10000 });
      await searchBar.fill('Parth_Playwright_test');
      await page.waitForTimeout(1500);
    ...
    await test.step('Select datasource from search results', async () => {
      ...
      const datasourceRow = page.locator('[role="row"], tr').filter({ hasText: 'Parth_Playwright_test' }).first();
    ...
    await test.step('Write SQL query and preview results', async () => {
      ...
      const sqlQuery = `SELECT * FROM "public"."cyber_secuitry`;
    ...
    await test.step('Fill Save Dataset dialog', async () => {
      ...
      const datasetName = `masterdata_test_parth_PostgreSQL_${timestamp}`;
    ...
    await test.step('Verify dataset was saved', async () => {
      ...
      try {
        ...
        // Step 17c: Search for the dataset at the TOP CENTER search bar
        console.log('Looking for search bar at the TOP CENTER...');
        
        // The search bar is at the top center with placeholder text like "Search files are folders in current section"
        const searchBar = page.locator('input[type="text"]').filter({ hasText: /search/i }).first();
        
        // Try multiple ways to find the search bar
        let foundSearchBar = false;
        
        if (await searchBar.isVisible({ timeout: 2000 }).catch(() => false)) {
          foundSearchBar = true;
          console.log('✓ Found search bar using filter');
        } else {
          // Try all text inputs and look for the one at the top
          const allSearchInputs = page.locator('input[type="text"], input[placeholder*="search" i], input[placeholder*="Search" i]');
          const count = await allSearchInputs.count();
          console.log(`Found ${count} search-like inputs`);
          
          if (count > 0) {
            // The top center search bar should be the first visible one
            for (let i = 0; i < count; i++) {
              try {
                const input = allSearchInputs.nth(i);
                const isVisible = await input.isVisible({ timeout: 1000 }).catch(() => false);
                if (isVisible) {
                  console.log(`Found visible search input at index ${i}`);
                  // Check if this input is in the top center area
                  const box = await input.boundingBox().catch(() => null);
                  if (box) {
                    console.log(`Search input position: top=${box.y}, left=${box.x}`);
                    if (box.y < 100) {  // Top area of the page
                      foundSearchBar = true;
                      break;
                    }
                  }
        ...
        if (foundSearchBar) {
          console.log('✓ Found search bar at top center - searching for masterdata_test_parth_PostgreSQL');
          await page.waitForTimeout(1000);
          ...
          if (searchBarIndex >= 0) {
            ...
            await topSearchBar.type('masterdata_test_parth_PostgreSQL', { delay: 50 });
            await page.waitForTimeout(2000);
            await waitAndScreenshot(page, '17c-search-executed');
            
            // Check if dataset appears in results
            const datasetNameText = page.locator('text=masterdata_test_parth_PostgreSQL');
            const isFound = await datasetNameText.isVisible({ timeout: 3000 }).catch(() => false);
            
            if (isFound) {
              console.log('✓✓✓ DATASET SAVED SUCCESSFULLY - Found masterdata_test_parth_PostgreSQL in search results');
