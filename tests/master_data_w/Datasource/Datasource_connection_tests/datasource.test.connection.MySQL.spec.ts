/**
 * datasource.test.connection.MySQL.spec.ts
 *
 * Datasource creation test — Master Data section
 * Creates a MySQL datasource with connection testing
 */
...
test.describe('Datasource Creation Flow', () => {
  ...
  test('should create a new datasource via Master Data > Datasources', async ({ page }) => {
    ...
    // Step 6: Search for MySQL in the top-right search bar
    await test.step('Search for MySQL datasource', async () => {
      const searchBar = page.getByPlaceholder('Search').filter({ visible: true }).first();
      await expect(searchBar).toBeVisible({ timeout: 10000 });
      await searchBar.fill('MySQL');
      await page.waitForTimeout(1000);

      const mysqlCard = page.getByRole('link', { name: /MySQL/i }).first();
      await expect(mysqlCard).toBeVisible({ timeout: 10000 });
      await mysqlCard.click();
      await waitAndScreenshot(page, '6-mysql-selected');
    });
    ...
    await test.step('Fill datasource credentials', async () => {
      ...
      // 1. Display Name
      await fillField('Display Name', 'Parth_Playwirght_mysql');
      await waitAndScreenshot(page, '7a-display-name-filled');

      // 2. Class
      await fillField('Class', 'com.mysql.cj.jdbc.Driver');
      await waitAndScreenshot(page, '7b-class-filled');

      // 3. Connection URL
      await fillField('Connection URL', 'jdbc:mysql://80.241.217.90:3306/CafeDB');
      await waitAndScreenshot(page, '7c-url-filled');

      // 4. Username
      await fillField('Username', 'aitest');
      await waitAndScreenshot(page, '7d-username-filled');

      // 5. Password
      await fillField('Password', 'password$01^&');
    ...
    await test.step('Search for datasource by name', async () => {
      const searchBar = page.getByPlaceholder('Search').filter({ visible: true }).first();
      await expect(searchBar).toBeVisible({ timeout: 10000 });
      await searchBar.fill('Parth_Playwirght_mysql');
      await page.waitForTimeout(1500);
    ...
    await test.step('Select datasource from search results', async () => {
      ...
      const datasourceRow = page.locator('[role="row"], tr').filter({ hasText: 'Parth_Playwirght_mysql' }).first();
    ...
    await test.step('Write SQL query and preview results', async () => {
      ...
      const sqlQuery = `SELECT \`log_id\` AS "log_id", \`customer_id\` AS "customer_id", \`communication_date\` AS "communication_date", \`communication_type\` AS "communication_type", \`employee_id\` AS "employee_id", \`subject\` AS "subject", \`communication_details\` AS "communication_details", \`follow_up_required\` AS "follow_up_required", \`follow_up_date\` AS "follow_up_date" FROM customer_communication_log`;
    ...
    await test.step('Fill Save Dataset dialog', async () => {
      ...
      const datasetName = `masterdata_test_parth_MySQL_${timestamp}`;
    ...
    await test.step('Verify dataset was saved', async () => {
      ...
      try {
        ...
        // Step 17c: Search for the folder (not the dataset directly)
        console.log('Searching for the Parth_Playwright_testing FOLDER...');
        await page.waitForTimeout(1000);
        ...
        if (searchBarIndex >= 0) {
          ...
          // Type the FOLDER name - search for the folder, not the dataset
          await topSearchBar.type('Parth_Playwright_testing', { delay: 50 });
          await page.waitForTimeout(2000);
          await waitAndScreenshot(page, '17c-folder-search-executed');
          console.log('✓ Searched for Parth_Playwright_testing folder');
          
          // Check if folder appears in results
          const folderElement = page.locator('text=Parth_Playwright_testing').first();
          if (await folderElement.isVisible({ timeout: 5000 }).catch(() => false)) {
            console.log('✓✓ Folder found in search results!');
            await waitAndScreenshot(page, '17d-folder-found');
          } else {
            console.log('✗ Folder not found in search results');
            await waitAndScreenshot(page, '17d-folder-not-found');
          }
        }
    ...
    // Step 18: Open Parth_Playwright_testing folder and search for dataset inside
    await test.step('Open Parth_Playwright_testing folder and search for dataset', async () => {
      console.log('Step 18: Opening Parth_Playwright_testing folder...');
      
      try {
        await page.waitForTimeout(1000);
        
        // Find and double-click Parth_Playwright_testing folder
        console.log('Looking for Parth_Playwright_testing folder in the datasets list...');
        const folderElement = page.locator('text=Parth_Playwright_testing').first();
        
        if (await folderElement.isVisible({ timeout: 5000 }).catch(() => false)) {
          console.log('✓ Found Parth_Playwright_testing folder - double-clicking it to open');
          await folderElement.dblclick({ force: true });
          await page.waitForTimeout(2000);
          await waitAndScreenshot(page, '18a-folder-opened');
          console.log('✓ Folder opened successfully');
          
          // Search for the dataset inside the folder
          console.log('Now searching for the dataset inside the folder...');
          await page.waitForTimeout(1000);
          
          // Get the search inputs after folder has been opened
          const searchInputs = page.locator('input[type="text"]');
          const count = await searchInputs.count();
          console.log(`Total text inputs after opening folder: ${count}`);
          ...
          if (searchBarIndex >= 0) {
            const searchBar = searchInputs.nth(searchBarIndex);
            
            // Clear and search for the dataset
            await searchBar.fill('');
            await page.waitForTimeout(200);
            
            // Type the dataset name using keyboard
            await searchBar.type('masterdata_test_parth_MySQL', { delay: 30 });
            await page.waitForTimeout(2000);
            await waitAndScreenshot(page, '18b-dataset-search-inside-folder');
            console.log('✓ Searched for dataset inside folder');
            
            // Check if dataset is found
            const datasetInFolder = page.locator('text=masterdata_test_parth_MySQL').first();
            if (await datasetInFolder.isVisible({ timeout: 5000 }).catch(() => false)) {
              console.log('✓✓✓ Dataset found inside Parth_Playwright_testing folder!');
              await waitAndScreenshot(page, '18c-dataset-found-in-folder');
              
              // Double-click on the dataset to open it
              console.log('Double-clicking dataset to open it...');
              await datasetInFolder.dblclick({ force: true });
              await page.waitForTimeout(2000);
              await waitAndScreenshot(page, '18d-dataset-opened');
              console.log('✓✓✓✓ Dataset opened successfully!');
            } else {
              console.log('✗ Dataset not found inside folder');
              await waitAndScreenshot(page, '18c-dataset-not-found-in-folder');
            }
          } else {
            console.log('✗ Could not find search bar inside folder');
            await waitAndScreenshot(page, '18b-search-bar-not-found');
          }
        } else {
          console.log('✗ Parth_Playwright_testing folder not found in datasets list');
          await waitAndScreenshot(page, '18a-folder-not-found');
        }
        
      } catch (error) {
        console.error('Error in Step 18:', String(error));
        await waitAndScreenshot(page, '18-error');
      }
    });
  });
});
