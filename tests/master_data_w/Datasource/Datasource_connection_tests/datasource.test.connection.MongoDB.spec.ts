/**
 * datasource.test.connection.MongoDB.spec.ts
 *
 * Datasource creation test — Master Data section
 * Creates a MongoDB datasource with connection testing
 */
...
test.describe('Datasource Creation Flow', () => {
  ...
  test('should create a new datasource via Master Data > Datasources', async ({ page }) => {
    ...
    // Step 6: Search for MongoDB in the top-right search bar
    await test.step('Search for MongoDB datasource', async () => {
      const searchBar = page.getByPlaceholder('Search').filter({ visible: true }).first();
      await expect(searchBar).toBeVisible({ timeout: 10000 });
      await searchBar.fill('MongoDB');
      await page.waitForTimeout(1000);

      const mongodbCard = page.getByRole('link', { name: /MongoDB/i }).first();
      await expect(mongodbCard).toBeVisible({ timeout: 10000 });
      await mongodbCard.click();
      await waitAndScreenshot(page, '6-mongodb-selected');
    });
    ...
    await test.step('Fill datasource credentials', async () => {
      ...
      // 1. Display Name
      await fillField('Display Name', 'Parth_Playwright_mongodb');
      await waitAndScreenshot(page, '7a-display-name-filled');

      // 2. Class
      await fillField('Class', 'mongodb.jdbc.MongoDriver');
      await waitAndScreenshot(page, '7b-class-filled');

      // 3. Connection URL
      await fillField('Connection URL', 'mongodb://178.105.6.171:27017/default?authSource=admin');
      ...
      // 5. Password
      await fillField('Password', 'y3xNL5YMk6HBwtvprDq9a0JOr9uK6i2kMQGdMmbiDsIbECXJ5ZaTUWUXz6W6OYac');
    ...
    await test.step('Search for datasource by name', async () => {
      const searchBar = page.getByPlaceholder('Search').filter({ visible: true }).first();
      await expect(searchBar).toBeVisible({ timeout: 10000 });
      await searchBar.fill('Parth_Playwright_mongodb');
      await page.waitForTimeout(1500);
    ...
    await test.step('Select datasource from search results', async () => {
      ...
      const datasourceRow = page.locator('[role="row"], tr').filter({ hasText: 'Parth_Playwright_mongodb' }).first();
    ...
    // Step 15: Write SQL/MongoDB query and generate output
    await test.step('Write query and preview results', async () => {
      // Wait for the dataset wizard/editor to load
      await page.waitForTimeout(2000);
      
      // MongoDB query
      const query = `db.init_collection.find({}, {'category': true})`;
      ...
      if (await editorContainer.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        if (await editorTextarea.isVisible({ timeout: 3000 }).catch(() => false)) {
          ...
          await editorTextarea.fill(query);
        } else {
      ...
      await waitAndScreenshot(page, '15-query-filled');
    ...
    await test.step('Fill Save Dataset dialog', async () => {
      ...
      const datasetName = `masterdata_test_parth_MongoDB_${timestamp}`;
    ...
    await test.step('Verify dataset was saved', async () => {
      ...
      try {
        ...
        console.log('Searching for the MongoDB dataset...');
        await page.waitForTimeout(1000);
        ...
        if (searchBarIndex >= 0) {
          ...
          await topSearchBar.type('masterdata_test_parth_MongoDB', { delay: 50 });
          await page.waitForTimeout(2000);
          await waitAndScreenshot(page, '17c-search-executed');
          
          // Check if dataset appears in results
          const datasetElement = page.locator('text=masterdata_test_parth_MongoDB').first();
        ...
        console.error('Error in Step 17:', String(error));
        await waitAndScreenshot(page, '17-error');
      }
    });
  });
});
